// RUN: nabla-opt %s --backward | FileCheck %s

func.func @avg_pool2d(%input : tensor<1x3x3x512xf32>) -> tensor<1x1x1x512xf32> {
  %result = "tosa.avg_pool2d"(%input) {
    kernel = array<i64: 3, 3>, pad = array<i64: 0, 0, 0, 0>, stride = array<i64: 1, 1>
...
// CHECK: %[[FACTOR:.*]] = "tosa.const"() {value = dense<0.111
// CHECK: %[[DINPUT:.*]] = "tosa.mul"(%[[DTARGET]], %[[FACTOR]])
// CHECK: return %[[DINPUT]]
