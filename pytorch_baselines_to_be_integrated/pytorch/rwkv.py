...
def parse_args():
    parser = argparse.ArgumentParser(description="Run RWKV model for inference or training.")
...
def initialize_model(model_name="RWKV/rwkv-5-world-1b5"):
...
def benchmark(model, tokenizer, mode, device, prompt, iterations=20):
    ...
    for idx in range(iterations):
        ...
        if mode == 'infer':
            num_tokens = outputs[0].size(0)  # Number of tokens generated
