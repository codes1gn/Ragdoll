...
def parse_args():
    parser = argparse.ArgumentParser(description="Run karpathy/tinyllama model for inference or training.")
...
# def initialize_model(model_name="/root/llama2_7b"):
def initialize_model(model_name="/root/.cache/huggingface/hub/models--karpathy--tinyllamas/snapshots/0bd21da7698eaf29a0d7de3992de8a46ef624add"):
    # Check if GPU is available
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    # Load the model and tokenizer
    tokenizer = AutoTokenizer.from_pretrained("/root/candle/llama2-c-utils/llama2.c/data/TinyStories_all_data", local_files_only=True)
    model = AutoModelForCausalLM.from_pretrained("/root/.cache/huggingface/hub/models--karpathy--tinyllamas/snapshots/0bd21da7698eaf29a0d7de3992de8a46ef624add", local_files_only=True)
...
def infer_step(model, inputs, device):
...
def benchmark(model, tokenizer, mode, device, prompt, iterations=20):
    ...
    for idx in range(iterations):
        ...
        if mode == 'infer':
            output, outputs = infer_step(model, inputs, device)
        elif mode == 'train':
