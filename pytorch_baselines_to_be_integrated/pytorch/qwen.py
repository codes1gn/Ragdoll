...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Qwen model for inference or training.")
    ...
    parser.add_argument('--model', type=str, choices=['qwen1.5-0.5b', 'qwen1.5-1.8b'], required=True,
                        help="Specify the model version: 'qwen1.5-0.5b' or 'qwen1.5-1.8b'.")
    return parser.parse_args()

# 2. Load the Model and Tokenizer (from Hugging Face)
def initialize_model(version="qwen1.5-0.5b"):
    # Check if GPU is available
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    # Define model name based on the version
    model_name = f"Qwen/{version}"  # Assuming the models are hosted under "Qwen1.5"
...
def main():
    ...
    model, tokenizer, device = initialize_model(version=args.model)
