...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Microsoft/phi model for inference or training.")
    ...
    parser.add_argument('--version', type=str, choices=['1', '1.5', '2', '3', '3m'], required=True,
                        help="Specify the version of the Microsoft/phi model to use.")
    return parser.parse_args()

# 2. Load the Model and Tokenizer
def initialize_model(version="1"):
    model_name = f"microsoft/phi-{version}"
...
def main():
    ...
    # Initialize model and tokenizer based on version
    model, tokenizer, device = initialize_model(version=args.version)
